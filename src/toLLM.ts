...
const formatValue = (value: unknown, indent: string = '', context: 'root' | 'array-item' | 'object-prop' = 'root'): string => {
  // Handle primitives
  ...
  if (typeof value === 'object' && value !== null) {
    ...
    if (isSmallValue(value, context)) {
      const pairs = keys.map(key => 
        JSON.stringify(key) + ':' + formatValue((value as Record<string, unknown>)[key], '', 'object-prop')
      );
      return '{' + pairs.join(',') + '}';
    }
