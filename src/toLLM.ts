...
const formatValue = (
  ...
  if (Array.isArray(value)) {
    ...
    if (isSmallValue(value, context)) return JSON.stringify(value);
 ...
 * but formatted to minimize token usage while adding some indentation
 * to remain human-readable.
 *
 * @param value The value to convert to JSON
 * @returns LLM-optimized JSON string
 */
export const toLLM = (value: unknown): string => formatValue(value, '', 'root');
